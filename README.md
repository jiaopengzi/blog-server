...
blog-server/
├─ blog-server              # 主程序
├─ blog-server.sig          # 主程序 Ed25519 签名文件, 启动完整性校验依赖
├─ boot                     # 启动包装器, 推荐用于生产启动
├─ config/                  # 运行配置目录
├─ templates/               # 模板资源
...
- `blog-server` 是主二进制程序.
- `blog-server.sig` 是主程序的 Ed25519 签名文件, 必须与主程序放在同一目录且不可改动, 否则主程序启动时完整性校验失败.
...
程序启动时会检测 PATH 中的 `ffmpeg` 与 `ffprobe`, 缺失将直接启动失败, 因此它们是必需依赖. 仓库中的容器镜像方案也是围绕 ffmpeg 运行时构建的.
...
- 裸机二进制部署生产环境时, 建议设置环境变量 `BLOG_SERVER_APP_PRO` 为非空值以启用生产行为; 容器镜像已内置该设置.
- 主程序启动时会校验同目录下的 `blog-server.sig` 签名文件, 请勿改动或遗漏.
...
如果 `config/app.yaml` 中的 `enable_api_doc` 设为 `true`, 应用会暴露在线接口文档入口.
...
- 本项目为闭源软件, 以二进制方式授权分发.
- 发布的主程序附带 Ed25519 签名, 启动时执行完整性校验, 用于防止二进制被篡改.
